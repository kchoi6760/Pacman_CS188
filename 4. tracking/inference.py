    ...
    for variable in variables[:3]:
        ...
        for x in range(X_RANGE):
            for y in range(Y_RANGE):
                pos.append((x, y))
        variableDomainsDict[variable] = pos


    dist_g0 = set()
    for pac in variableDomainsDict[PAC]:
        for ghost in variableDomainsDict[GHOST0]:
            noise_pos = manhattanDistance(ghost, pac) + MAX_NOISE
            noise_neg = manhattanDistance(ghost, pac) - MAX_NOISE
            if noise_pos >= 0:
                dist_g0.add(noise_pos)
            if noise_neg >= 0:
                dist_g0.add(noise_neg)
    variableDomainsDict[OBS0] = list(dist_g0)

    dist_g1 = set()
    for pac in variableDomainsDict[PAC]:
        for ghost in variableDomainsDict[GHOST1]:
            noise_pos = manhattanDistance(ghost, pac) + MAX_NOISE
            noise_neg = manhattanDistance(ghost, pac) - MAX_NOISE
            if noise_pos >= 0:
                dist_g1.add(noise_pos)
            if noise_neg >= 0:
                dist_g1.add(noise_neg)
    variableDomainsDict[OBS1] = list(dist_g1)
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for aElim in eliminationOrder:
            factors, aFactor = joinFactorsByVariable(factors, aElim)
            if len(aFactor.unconditionedVariables()) != 1:
                eliminated = eliminate(aFactor, aElim)
                factors.append(eliminated)
        res = joinFactors(factors)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            observed_prob = self.getObservationProb(observation, pac_pos, pos, jail_pos)
            ddist[pos] = observed_prob * self.beliefs[pos]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            for key, val in self.getPositionDistribution(gameState, pos).items():
                ddist[key] += self.beliefs[pos] * val
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        particle_spacer = len(self.legalPositions) / self.numParticles
        even_spacing = range(self.numParticles)

        for i in even_spacing: 
            pos = self.legalPositions[int(i * particle_spacer)]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        ddist = self.getBeliefDistribution()

        for part in ddist:
            prob = self.getObservationProb(observation, gameState.getPacmanPosition(), part, self.getJailPosition())
            ddist[part] *= prob
        ...
        self.particles = []
        for useless in range(self.numParticles):
            self.particles.append(ddist.sample())
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            ...
            for key, val in newPosDist.items():
                newDistribution[key] += bdist[pos] * val

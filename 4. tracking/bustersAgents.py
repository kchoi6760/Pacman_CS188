...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for dist in livingGhostPositionDistributions: 
            likely_pos = dist.argMax()
            distance = self.distancer.getDistance(pacmanPosition, likely_pos)

            if distance < minDistance: 
                minDistance, minPos = distance, likely_pos

        minDistance_post = float("inf") 
        for act in legal: 
            sucPos = Actions.getSuccessor(pacmanPosition, act) 
            post_distance = self.distancer.getDistance(minPos, sucPos) 

            if post_distance < minDistance_post: 
                minDistance_post, best_action = post_distance, act

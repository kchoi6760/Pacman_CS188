...
def sentence3() -> Expr:
    ...
    expr1 = Expr('<=>', PacmanAlive_1, ((PacmanAlive_0 & ~PacmanKilled_0) | (~PacmanAlive_0 & PacmanBorn_0)))
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    sent = premise & ~conclusion
    return not findModel(sent)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    combos_literals = [disjoin(~literal[0], ~literal[1]) for literal in itertools.combinations(literals, 2)]
    return conjoin(combos_literals)
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    pacphysics_sentences.append(exactlyOne([PropSymbolExpr(d, time=t) for d in DIRECTIONS]))

    if sensorModel != None:
        pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))

    if t and walls_grid and successorAxioms:
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        the_goal = PropSymbolExpr(pacman_str, xg, yg, time=t)
        the_model = findModel(conjoin(KB) & the_goal)
        if the_model:
            return extractActionSequence(the_model, actions)
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    for foodx, foody in food:
        KB.append(PropSymbolExpr(food_str, foodx, foody, time=0))

    for t in range(50):
        print(t)
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_wall_coords]))
        the_goal = [~PropSymbolExpr(food_str, foodx, foody, time=t) for foodx, foody, in food]
        the_model = findModel(conjoin(KB + the_goal))
        if the_model:
            return extractActionSequence(the_model, actions)
        ...
        for foodx, foody in food:
            food_t = PropSymbolExpr(food_str, foodx, foody, time=t)
            food_t1 = PropSymbolExpr(food_str, foodx, foody, time=t+1)
            pacman_t = PropSymbolExpr(pacman_str, foodx, foody, time=t)
            KB.append((~pacman_t & food_t) >> food_t1)
            KB.append((pacman_t & food_t) >> ~food_t1)
    "*** END YOUR CODE HERE ***"

def add_PAP_KB(KB, agent, t, all_coords, non_outer_wall_coords, walls_grid):
...
def possible_loc_KB(KB, t, x, y, possible_locations):
...
def provable_walls(KB, x, y, walls_grid):
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        possible_locations = []
        add_PAP_KB(KB, agent, t, all_coords, non_outer_wall_coords, walls_grid)
        for wx, wy in non_outer_wall_coords:
            possible_loc_KB(KB, t, wx, wy, possible_locations)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        add_PAP_KB(KB, agent, t, all_coords, non_outer_wall_coords, known_map)

        for wx, wy in non_outer_wall_coords:
            provable_walls(KB, wx, wy, known_map)
...
def slam(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    def add_PAP_KB_SLAM(KB, agent, t, all_coords, non_outer_wall_coords, walls_grid):
    ...
    for t in range(agent.num_timesteps):
        possible_locations = []
        add_PAP_KB_SLAM(KB, agent, t, all_coords, non_outer_wall_coords, known_map)
